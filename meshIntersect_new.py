...
# print(inflines[0])
plt.scatter(spheres[:, 1], spheres[:, 0])
...
for i in range(len(inflines)):
    ...
    start=time.time()
    xlim = spheres[np.where(np.logical_and(np.greater_equal(xmax,spheres[:,1]),np.less_equal(xmin, spheres[:,1])))]
    lims = xlim[np.where(np.logical_and(np.greater_equal(ymax, xlim[:,0]),np.less_equal(ymin, xlim[:,0])))]
    end = time.time()
    # print('cropped sphere points to image corners in: ', end-start, ' seconds')
#   plt.scatter(spheres[:,0],spheres[:,2])
#     plt.scatter(lims[:,1],lims[:,0])
#     plt.show()
    # print(np.shape(lims))
    # print(lims)
    ...
    num_ints = 0
    for sp in lims:
        x,y,z = sp[1],sp[0], sp[2]#[sp[0],sp[2],sp[1]]
        # print(x,y,z)
...
np.save('/home/nader/scratch/lobsterpts_3d_new', intx)

np.save('/home/nader/scratch/lobsterpt_indices_new', det_indices)